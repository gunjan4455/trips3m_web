...
import styles from './willBook.cm.scss';
...
class WillBook extends React.Component {
  ...
  handleChange(event) {
    this.props.input.onChange({ option_id: event.target.value, customization_id: '43' });
  }
    ...
    return (
      <div className="clearfix will-book-comp">
        <div className="iblock wfull">
          <p className="f14p fw7 sfc6 m0 iblock f14">I Will Book<em className="pfc2">*</em></p>
          <FilterClear
          ...
          <div className="row row-8">
            {
              Object.keys(willBookPrefCustomizations).map((key, index) =>
                <div className={`col-xs-6 p8 will-book-con-options at_edit_willbook ${styles.willBook}`} key={key}>
                  <input
                    type="radio"
                  ...
                  <label htmlFor={`radio_button_${key}_${index}`} className="cursorP">
